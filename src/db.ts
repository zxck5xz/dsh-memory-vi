...
function toMemory(row: DbRow): MemoryRow {
...
export function createStore(root: string) {
  ...
  function save(project: string, key: string, content: string, tags: string[]): MemoryRow {
    const updated_at = new Date().toISOString()
    upsert.run(project, key, content, JSON.stringify(tags), updated_at)
    return { project, key, content, tags, updated_at }
  }

  function get(project: string, key: string): MemoryRow | null {
    const row = select.get(project, key) as DbRow | undefined
    return row ? toMemory(row) : null
  }

  function search(project: string, query: string, tags: string[], limit: number): MemoryRow[] {
    const like = `%${escapeLike(query)}%`
    const rows = searchStmt.all(project, like, like) as unknown as DbRow[]
